...
class BaseGameSession {
  ...
  constructor(id, settings = {}, broadcastFn = null) {
    ...
    this.ownerId = null;
    this.interruptingPlayer = null; // Player ID who is currently interrupting
  ...
  tick() {
    ...
    if (this.interruptingPlayer) {
      const interruptingPlayer = this.players.find(p => p.id === this.interruptingPlayer);
  ...
  interrupt(playerId) {
    ...
    if (this.interruptingPlayer !== null) return;

    if (player.id === this.activePlayer) return;

    this.interruptingPlayer = playerId;
    this.broadcastState();
  }
  ...
  getState() {
    ...
    return {
      ...
      interruptingPlayer: this.interruptingPlayer,
      ...this.getModeState(),
    };
  }
  ...
  static fromState(state, broadcastFn = null) {
    ...
    session.interruptingPlayer = state.interruptingPlayer || null;
