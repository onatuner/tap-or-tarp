...
class SessionStorage {
  ...
  initialize() {
    ...
    // Migration: Add is_closed column if missing (for existing databases)
    // Must run BEFORE index creation that references is_closed
    const columns = this.db.pragma("table_info(sessions)");
    const hasIsClosed = columns.some(col => col.name === "is_closed");
    if (!hasIsClosed) {
      this.db.exec("ALTER TABLE sessions ADD COLUMN is_closed INTEGER DEFAULT 0");
    }

    // Create indexes for cleanup queries (after migration ensures columns exist)
    this.db.exec(`
    ...
    this.statements = {
      save: this.db.prepare(`
				INSERT OR REPLACE INTO sessions (id, state, created_at, updated_at, is_closed)
      ...
      loadActive: this.db.prepare("SELECT id, state FROM sessions WHERE is_closed = 0"),
      delete: this.db.prepare("DELETE FROM sessions WHERE id = ?"),
      cleanup: this.db.prepare("DELETE FROM sessions WHERE updated_at < ?"),
      cleanupClosed: this.db.prepare(
        "DELETE FROM sessions WHERE is_closed = 1 AND updated_at < ?"
      ),
      closeSession: this.db.prepare("UPDATE sessions SET is_closed = 1, updated_at = ? WHERE id = ?"),
      count: this.db.prepare("SELECT COUNT(*) as count FROM sessions"),
      countActive: this.db.prepare("SELECT COUNT(*) as count FROM sessions WHERE is_closed = 0"),
  ...
  save(id, sessionState) {
    ...
    const now = Date.now();
    const isClosed = sessionState.isClosed ? 1 : 0;
    this.statements.save.run(id, state, id, now, now, isClosed);
  }
  ...
  saveBatch(sessions) {
    ...
    const saveTransaction = this.db.transaction(sessionsToSave => {
      for (const { id, state } of sessionsToSave) {
        ...
        const isClosed = state.isClosed ? 1 : 0;
        this.statements.save.run(id, stateJson, id, now, now, isClosed);
      }
  ...
  /**
   * Load all active (non-closed) sessions from the database
   * @returns {Array} Array of {id, state} objects for active sessions only
   */
  loadActiveSessions() {
    if (!this.db) return [];

    const rows = this.statements.loadActive.all();
    return rows.map(row => ({
      id: row.id,
      state: JSON.parse(row.state),
    }));
  ...
  closeSession(id) {
    if (!this.db) return;
    this.statements.closeSession.run(Date.now(), id);
  }
   ...
   * @returns {number} Number of deleted sessions
   */
  deleteClosedSessions(maxAge = 24 * 60 * 60 * 1000) {
    if (!this.db) return 0;

    const cutoff = Date.now() - maxAge;
    const result = this.statements.cleanupClosed.run(cutoff);
    return result.changes;
  }

  /**
   * Get the number of active (non-closed) sessions
   * @returns {number} Active session count
   */
  countActiveSessions() {
    if (!this.db) return 0;
    return this.statements.countActive.get().count;
  }
...
class MemoryStorage {
  ...
  /**
   * Load all active (non-closed) sessions from memory
   * @returns {Array} Array of {id, state} objects for active sessions only
   */
  loadActiveSessions() {
    return Array.from(this.sessions.entries())
      .filter(([id, data]) => !data.state.isClosed)
      .map(([id, data]) => ({
        id,
        state: data.state,
      }));
  ...
  deleteClosedSessions(maxAge = 24 * 60 * 60 * 1000) {
    ...
    let deleted = 0;
    for (const [id, data] of this.sessions.entries()) {
      if (data.state.isClosed && data.updatedAt < cutoff) {
        this.sessions.delete(id);
        deleted++;
      }
    }
   ...
   */
  countActiveSessions() {
    return Array.from(this.sessions.values()).filter(data => !data.state.isClosed).length;
  }
