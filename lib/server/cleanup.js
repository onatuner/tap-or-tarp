...
async function cleanupSessions() {
  ...
  // Also cleanup old closed sessions from storage (24 hours old)
  if (serverState.storage && serverState.storage.deleteClosedSessions) {
    try {
      ...
      if (deletedClosed > 0) {
        logger.info({ deletedClosed }, "Cleaned up old closed sessions from storage");
      }
    } catch (error) {
      logger.error({ error: error.message }, "Failed to cleanup closed sessions");
    }
  }
