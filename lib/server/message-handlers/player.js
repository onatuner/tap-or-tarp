...
async function handleUpdateSettings(ws, data) {
  ...
  try {
    await withGameLock(ws.gameId, async () => {
      ...
      if (data.bonusTime !== undefined) {
        // Validate: 0 to 5 minutes (0-300000ms)
        const bonusTime = Math.max(0, Math.min(data.bonusTime, CONSTANTS.MAX_BONUS_TIME));
        session.settings.bonusTime = bonusTime;
        settingsChanged = true;
      }

      if (data.timeoutPenaltyLives !== undefined) {
        // Validate: 0 to 20 lives penalty
        session.settings.timeoutPenaltyLives = Math.max(0, Math.min(data.timeoutPenaltyLives, 20));
        settingsChanged = true;
      }

      if (data.timeoutPenaltyDrunk !== undefined) {
        // Validate: 0 to 20 drunk penalty
        session.settings.timeoutPenaltyDrunk = Math.max(0, Math.min(data.timeoutPenaltyDrunk, 20));
        settingsChanged = true;
      }

      if (data.timeoutBonusTime !== undefined) {
        // Validate: 0 to 5 minutes (0-300000ms)
        const bonusTime = Math.max(0, Math.min(data.timeoutBonusTime, CONSTANTS.MAX_BONUS_TIME));
        session.settings.timeoutBonusTime = bonusTime;
        settingsChanged = true;
      }
