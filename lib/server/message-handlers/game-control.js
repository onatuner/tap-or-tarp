...
async function handleInterrupt(ws, data) {
  ...
  try {
    await withGameLock(ws.gameId, async () => {
      ...
      if (myPlayer.id === session.activePlayer) {
        safeSend(
          ws,
          JSON.stringify({
            type: "error",
            data: { message: "You cannot interrupt while you are the active player" },
          })
        );
        return;
      }

      if (session.interruptingPlayer !== null) {
        safeSend(
          ws,
          JSON.stringify({
            type: "error",
            data: { message: "Another player is already interrupting" },
          })
        );
        return;
      }
...
async function handlePassPriority(ws, data) {
  ...
  try {
    await withGameLock(ws.gameId, async () => {
      ...
      if (session.interruptingPlayer !== myPlayer.id) {
        safeSend(
          ws,
          JSON.stringify({
            type: "error",
            data: { message: "You are not the interrupting player" },
          })
        );
        return;
      }
...
async function handleRenameGame(ws, data) {
  ...
  try {
    await withGameLock(ws.gameId, async () => {
      ...
      // Validate and update game name
      const newName = data.name?.trim() || "Game";
